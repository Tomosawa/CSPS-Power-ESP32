...
static const int blink_do =  45, PSON_do =  37, I2C_enable_do = 15;
...
U8X8_SSD1306_128X64_NONAME_SW_I2C u8x8(/* clock=*/ 2, /* data=*/ 42, /* reset=*/ U8X8_PIN_NONE);   // OLEDs without Reset of the Display
//U8X8_SSD1306_128X64_NONAME_HW_I2C u8x8(/* clock=*/ 2, /* data=*/ 42, /* reset=*/ U8X8_PIN_NONE);
#elif LCD_DISPLAY == 16
...
void setup() {
 ...
 //Wire1.begin(42,2,100000ul);
  ...
  psu.init(PSON_do,I2C_enable_do,OUTPUT_DIRECTION,0x5F,&DebugSerial,&Wire);
  //psu.init(PSON_do,I2C_enable_do,OUTPUT_DIRECTION,0x4f,&DebugSerial,&Wire1);
#else
  psu.init(PSON_do,I2C_enable_do,OUTPUT_DIRECTION,0x5F,&DebugSerial);
  //psu.init(PSON_do,I2C_enable_do,OUTPUT_DIRECTION,0x4f,&DebugSerial);
#endif
...
struct tm         *local_tm;
  ...
  if ((run_secs > 3)&&
    ...
    case 5:
      ...
      // text[i] = 0;
      my_ip      = WiFi.localIP();
      sprintf(text,"%d.%d.%d.%d",my_ip[0],my_ip[1],my_ip[2],my_ip[3]);
      u8x8.drawString(0,display_phase,text);
      break;
